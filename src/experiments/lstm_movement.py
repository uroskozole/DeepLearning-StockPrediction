...
from models.lstm import LSTMMovement
...
def parse_args_():
    ...
    args.results_path = f"results/movement/lstm/{args.time_jump}/seq_len{args.seq_len}_epochs{args.epochs}_lr{args.learning_rate}_scheduler{args.scheduler}/"
...
def train_lstm(X, y, val_df, model, loss_function, optimizer, args):
    ...
    for epoch in range(args.epochs):
        ...
        val_preds = np.array([int(model(x)[-1].detach().numpy() > .49999999) for x in X_val])
        val_y = y_val[:, -1]
        validation_accuracy = np.mean(np.array(val_preds) == val_y.detach().numpy())
                
        print(f'Epoch: {epoch}, train logloss: {np.mean(np.array(losses))}, validation accuracy: {validation_accuracy}')
...
if __name__ == "__main__":
    ...
    model = LSTMMovement(hidden_dim=128, input_dim=len(train_df.columns), reduce=False)#.to(device)
